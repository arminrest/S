...
class txttableclass:
    ...
    def cleardefs(self):
        ...
        self.indicator4headerpattern='^\s*#'
...
#        print 'GGGGGGGGG', type(cols)
    ...
    def selectlckeys(self,keys=None,photcodes=None,dophottypes=None,onlyforced=0,onlynonforced=0,MJDmin=None,MJDmax=None,skipbadim=0,onlybadim=0,Nmaskmax=None,skipifpeakmasked=None):
        if keys == None:
            keys = self.rowkeys()
        #print('BBBBBB',keys,photcodes,dophottypes,onlyforced,onlynonforced,'VVV',MJDmin,MJDmax,skipbadim,onlybadim,Nmaskmax,skipifpeakmasked)
        ...
        for lckey in keys:
            ...
            #    print('lckey',lckey,lcpc,photcodes,self.getentry(lckey,'type') & FORCEDPHOTFLAG)
            #else:
            #    print('lckey',lckey,'AAAAAAAAAAAAAAhhhhhh')
            ...
            #print('lckey',lckey,'test1',MJDmin,MJDmax,self.getentry(lckey,'MJD'))
            # check for MJD
    ...
    def straightline_errorcut(self,keys,xcol,ycol,offset,slope,x0=0.0,dycol=None,dycol2=None,Nsigma=3.0,dyval=1.0,verbose=0):
        ...
        if dycol2 == None:
            ...
            if Nused<1:
                if verbose:
                    print(('WARNING: no data to calculate straigh line for xcol %s and ycol %s!' % (xcol,ycol)))
            ...
            if val2!=None:
                ...
                if raiseErrorFlag:
                    ...
                    if skipifnotexist:
                        ...
                        return(6)
        if keys1all==None:
            keys1notmatched=AnotB(t1.allrowkeys,unique(keys1matched))
        else:
            keys1notmatched=AnotB(keys1all,unique(keys1matched))
...
class paramfileclass(txttableclass):
    """
    This class parses through the parameter file
    """
    def __init__(self, filename=None,**keys):
        txttableclass.__init__(self,**keys)
        self.requireentry=0
        self.expandtabs=1

        # define the columns of the parameter file
        self.paramnamecol='paramname'
        self.paramvalcol='paramval'
        # This is an exception: use the parameter name column as rowkey
        self.col4rowkey=self.paramnamecol

        # compile the pattern to kick out comments
        self.commentpattern=re.compile('^#.*|\s*###.*')

        # compile the pattern to fine environment variables
        self.envvarpattern=re.compile('\$\((\w+)\)')

        # compile the pattern to replace placeholders of other parameters
        self.paramplaceholderpattern=re.compile('\$\[(\w+)\]')
...
#        self.debug = True
        ...
        self.hexpattern = re.compile('^0x')

        self.commapattern = re.compile(',')



    def loadfile(self,filename,addflag=0):
        if self.debug:
            print(("PIPECLASSES: 1) Cols in txttable object: ",  self.cols))
            print(("PIPECLASSES: Loading texttable file: ", filename))
        try:
            texttable.txttableclass.loadfile(self,filename,addflag=addflag,maxsplit=1)
        except:
            print("PIPECLASSES::loadfile found exception in 'texttable.txttableclass.loadfile'")
            raise
        if self.debug:
            print(("PIPECLASSES: Keys in txttable object: ",  list(self.data.keys())))
            print(("PIPECLASSES: Cols in txttable object: ",  self.cols))
            print(("PIPECLASSES: self.paramnamecol, self.paramvolcol ", self.paramnamecol, self.paramvalcol))
        if not self.colsexist([self.paramnamecol,self.paramvalcol]):
            raise RuntimeError('Couldn\'t find the parameter name column "%s" and parameter value column "%s" in file %s' % (self.paramnamecol,self.paramvalcol,filename))
        for key in self.allrowkeys:
            if self.getentry(key,self.paramvalcol,verbose=False) is None:
                self.setentry(key,self.paramvalcol,None)
            else:
                self.setentry(key,self.paramvalcol,self.commentpattern.sub('',"%s" % (self.getentry(key,self.paramvalcol))))
                self.subenvvarplaceholder(key)

        # get the parameter placeholders. repeat until there are no substitution anymore
        repeatflag = 1
        while repeatflag:
            repeatflag = 0
            for key in self.allrowkeys:
                repeatflag |= self.subparamplaceholder(key)
    def subenvvarplaceholder(self,rowkey):
        if self.colinfo[self.paramvalcol]['type']!='s':
            return 0
        envvarnames=self.envvarpattern.findall(self.getentry(rowkey,self.paramvalcol))
        if envvarnames:
            for name in envvarnames:
                envval=os.environ[name]
                subpattern='\$\(%s\)' % (name)
                self.setentry(rowkey,self.paramvalcol,re.sub(subpattern,envval,self.getentry(rowkey,self.paramvalcol)))

    def subparamplaceholder(self,rowkey):
        if self.colinfo[self.paramvalcol]['type']!='s':
            return 0
        if self.getentry(rowkey,self.paramvalcol)==None:
            return 0
        paramplaceholders=self.paramplaceholderpattern.findall(self.getentry(rowkey,self.paramvalcol))
        if paramplaceholders:
            for name in paramplaceholders:
                paramval = self.get(name)
                subpattern='\$\[%s\]' % (name)
                if paramval == None:
                    if re.match('^%s$' % subpattern,self.getentry(rowkey,self.paramvalcol)):
                        self.setentry(rowkey,self.paramvalcol,paramval)
                        continue
                    else:
                        paramval = ""
                self.setentry(rowkey,self.paramvalcol,re.sub(subpattern,paramval,self.getentry(rowkey,self.paramvalcol)))
            return(1)
        return(0)

    def paramexists(self,paramname):
        if paramname in self.data:
            return(1)
        return(0)

    def get(self,paramname):
        if paramname not in self.data:
            raise RuntimeError('ERROR: parameter "%s" does not exist in the paramter file!!!' % paramname)
        return(self.getentry(paramname,self.paramvalcol))
        #try:
        #    return(self.getentry(paramname,self.paramvalcol))
        #except:
        #    return None
    def get_asfloat(self,paramname,default=None):
        try:
            return(float(self.get(paramname)))
        except:
            return default
    def get_asint(self,paramname,default=None):
        try:
            return(int(self.get(paramname)))
        except:
            return default
    def get_ashex(self,paramname,default=None):
        val = self.get(paramname)
        try:
            if self.hexpattern.search(val):
                return(int(val,16))
            else:
                return(int(val))
        except:
            return default
    def getlist(self,paramname,default=None):
        val = self.get(paramname)
        try:
            return(self.commapattern.split(val))
        except:
            return default

    def getintlist(self,paramname,default=None):
        vallist = self.getlist(paramname,default=default)
        if vallist == None: return default
        try:
    ...
    def getfloatlist(self,paramname,default=None):
        vallist = self.getlist(paramname,default=default)
        if vallist == None: return default
        try:
            for i in range(len(vallist)):
    ...
    def gethexlist(self,paramname,default=None):
        vallist = self.getlist(paramname,default=default)
        if vallist == None: return default
        try:
            for i in range(len(vallist)):
                vallist[i] = tools.hex2int(vallist[i])
            return(vallist)
        except:
            return default

    def set(self,paramname,val):
        if (paramname in self.data):
            self.setentry(paramname,self.paramvalcol,val)
        else:
            self.newrow({self.paramnamecol:paramname,self.paramvalcol:val})
